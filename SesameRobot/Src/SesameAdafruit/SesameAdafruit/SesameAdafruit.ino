...
void setup() {
  ...
  for (byte addr = 1; addr < 127; addr++) {
    ...
    if (error == 0) {
      ...
      if (addr == 0x40) Serial.print(F(" (PCA9685 Servo)"));
      Serial.println();
      deviceCount++;
    }
  }
  ...
  // OLED Init
  Serial.print(F("Initializing OLED at 0x"));
  Serial.print(OLED_I2C_ADDR, HEX);
  Serial.println(F("..."));

  if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_I2C_ADDR)) {
    Serial.println(F("ERROR: SSD1306 allocation failed!"));
    Serial.println(F("Check: Is OLED connected? Correct address (0x3C vs 0x3D)?"));
    while (1) {
      delay(1000);  // Halt but allow serial output
    }
  }
  ...
  display.display();
  Serial.println(F("OLED test pattern displayed."));
  ...
  // PCA9685 Servo FeatherWing Init
  Serial.println(F("Initializing PCA9685 Servo FeatherWing at 0x40..."));
  pwm.begin();
