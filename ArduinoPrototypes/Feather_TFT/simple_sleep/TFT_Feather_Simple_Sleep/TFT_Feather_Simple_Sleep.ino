...
void goToSleep() {
  // Configure D2 as wakeup source
  esp_sleep_enable_ext0_wakeup((gpio_num_t)D2Pin, 1); // Wake on HIGH signal
  Serial.flush(); // print last messages
...
void loop(){
  ...
  if(d1CurrentlyPressed != d1Pressed)
  {
    ...
    if(d1CurrentlyPressed)
    {
      ...
      delay(1000); // allow message to flush
      goToSleep();
    }
    else
    {
